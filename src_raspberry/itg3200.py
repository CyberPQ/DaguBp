...
class SensorITG3200(object):
	...
	def read_data(self):
		...
		as signed float in degre per second.
		"""
		ITG3200_GYRO_LSB_per_DEG_SEC = 14.375 # LSB / (°/s)
		
		# get value from sensor (with offset correction)
		x = int_sw_swap(self.bus.read_word_data(self.addr, 0x1d))
		y = int_sw_swap(self.bus.read_word_data(self.addr, 0x1f))
		z = int_sw_swap(self.bus.read_word_data(self.addr, 0x21))
		
		# Convert to degres/s :	  14.375 LSB / (°/s)   (with offset correction)
		gx = (float(x) / ITG3200_GYRO_LSB_per_DEG_SEC) + self.offx
		gy = (float(y) / ITG3200_GYRO_LSB_per_DEG_SEC) + self.offy
		gz = (float(z) / ITG3200_GYRO_LSB_per_DEG_SEC) + self.offz
	
		return (gx, gy, gz)

if __name__ == '__main__':
	import time
	sensor = SensorITG3200(1, 0x69) # update with your bus number and address
	sensor.default_init()
	time.sleep(0.1)
	
	for i in range(0,1000) :
		gx, gy, gz = sensor.read_data()
		print "{:+10.4f} {:+10.4f} {:+10.4f}".format(gx, gy, gz)
